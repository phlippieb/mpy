...
def _store(pso_1_name, pso_2_name, swarm_size, benchmark_name, dimensionality, num_iterations, rank):
    ...
    droc_rank_between_psos_table.store(
        pso_1_name, pso_2_name, swarm_size, benchmark_name, dimensionality, num_iterations, rank)
    # Store inverted result for inverted order (pso_2_name, pso_1_name):
    droc_rank_between_psos_table.store(
        pso_2_name, pso_1_name, swarm_size, benchmark_name, dimensionality, num_iterations, rank * -1)
...
def _calculate(pso_1_name, pso_2_name, swarm_size, benchmark_name, dimensionality, num_iterations, verbose=False, benchmark=False):
    ...
    pso_1_drocs = [drocs.get(pso_1_name, swarm_size, benchmark_name, dimensionality, num_iterations,
                             experiment_num, verbose=verbose, force_calculation=benchmark) for experiment_num in experiment_nums]
    pso_2_drocs = [drocs.get(pso_2_name, swarm_size, benchmark_name, dimensionality, num_iterations,
                             experiment_num, verbose=verbose, force_calculation=benchmark) for experiment_num in experiment_nums]
