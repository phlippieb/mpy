...
def store(benchmark_name, dimensionality, epsilon, step_size_fraction, experiment, measurement):
    db.cursor.execute(
        'INSERT INTO PN (benchmark_name, dimensionality, epsilon, step_size_fraction, experiment, measurement)' +
        'VALUES (%s, %s, %s, %s, %s, %s)' +
...
def fetch(benchmark_name, dimensionality, epsilon, step_size_fraction, experiment):
    db.cursor.execute(
        'SELECT measurement FROM PN WHERE benchmark_name=%s AND dimensionality=%s AND epsilon=%s AND step_size_fraction=%s AND experiment=%s',
        (benchmark_name, dimensionality, epsilon, step_size_fraction, experiment))
    rows = db.cursor.fetchall()
    if len(rows) < 1:
        return None
    elif len(rows) > 1:
        warnings.warn("Multiple results found when fetching PN with benchmark_name={} and dimensionality={} and epsilon={} and step_size_fraction={} and experiment={}".format(
            benchmark_name, dimensionality, epsilon, step_size_fraction, experiment))
