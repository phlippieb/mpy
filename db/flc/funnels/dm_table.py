...
def store(benchmark_name, dimensionality, experiment, measurement):
    db.cursor.execute(
        'INSERT INTO DM (benchmark_name, dimensionality, experiment, measurement)' +
        'VALUES (%s, %s, %s, %s)' +
...
def fetch(benchmark_name, dimensionality, experiment):
    db.cursor.execute(
        'SELECT measurement FROM DM WHERE benchmark_name=%s AND dimensionality=%s AND experiment=%s',
