...
def store(benchmark_name, dimensionality, experiment, measurement):
    db.cursor.execute(
        'INSERT INTO FCI_cog (benchmark_name, dimensionality, experiment, fci_cog)' +
        'VALUES (%s, %s, %s, %s)' +
        'ON CONFLICT (benchmark_name, dimensionality, experiment)' +
        'DO UPDATE SET fci_cog = %s',
...
def fetch(benchmark_name, dimensionality, experiment):
    db.cursor.execute(
        'SELECT fci_cog FROM FCI_cog WHERE benchmark_name=%s AND dimensionality=%s AND experiment=%s',
        (benchmark_name, dimensionality, experiment))
    rows = db.cursor.fetchall()
    if len(rows) < 1:
        return None
    elif len(rows) > 1:
        warnings.warn("Multiple results found when fetching FCI_cog with benchmark_name={} and dimensionality={} and experiment={}".format(
            benchmark_name, dimensionality, experiment))
