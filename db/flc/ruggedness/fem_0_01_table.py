...
def store(benchmark_name, dimensionality, experiment, measurement):
    db.cursor.execute(
        'INSERT INTO FEM_0_01 (benchmark_name, dimensionality, experiment, measurement)' +
        'VALUES (%s, %s, %s, %s)' +
...
def fetch(benchmark_name, dimensionality, experiment):
    db.cursor.execute(
        'SELECT measurement FROM FEM_0_01 WHERE benchmark_name=%s AND dimensionality=%s AND experiment=%s',
        (benchmark_name, dimensionality, experiment))
    rows = db.cursor.fetchall()
    if len(rows) < 1:
        return None
    elif len(rows) > 1:
        warnings.warn("Multiple results found when fetching FEM_0_01 with benchmark_name={} and dimensionality={} and experiment={}".format(
            benchmark_name, dimensionality, experiment))
